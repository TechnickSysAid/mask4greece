 ...
 */
class pdf_moon_invoicegr1 extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "ΤΙΜΟΛΟΓΙΟ-ΠΑΡΟΧΗΣ ΥΠΗΡΕΣΙΩΝ";
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					pdf_writelinedescgr($pdf,$object,$i,$outputlangs,$this->posxtva-$curX,3,$curX,$curY,$hideref,$hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedescgr($pdf,$object,$i,$outputlangs,$this->posxtva-$curX,4,$curX,$curY,$hideref,$hidedesc);
						$pageposafter=$pdf->getPage();
						$posyafter=$pdf->GetY();
					...
					$pdf->MultiCell($this->posxqty-$this->posxup-0.8, 3, $up_excl_tax, 0, 'R', 0);
                                        ...
                                        // Pieces and units
                                        //$labelproductservice=pdf_getlinedescgr($object,$i,$outputlangs,$hideref,$hidedesc,$issupplierline,$qty1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,0);
		$title=$outputlangs->transnoentities("Invoicegr1");
		...
		if ($object->type != 2)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,0);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DateEcheancegr").": " . dol_print_date($object->date_lim_reglement,"day",false,$outputlangs,true), '', 'L');
		}
