	...
	if ($mode == 'source')
	{
		...
		if (empty($conf->global->MAIN_PDF_DISABLESOURCEDETAILS))
		{
			...
			if ($sourcecompany->tva_intra) $stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->transnoentities("VATIntraShort").": ".$outputlangs->convToOutputCharset($sourcecompany->tva_intra);
                        // Prof Id 1
			if ($sourcecompany->idprof1) $stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->transnoentities("ProfId1").": ".$outputlangs->convToOutputCharset($sourcecompany->idprof1);
                        // Prof Id 2
			if ($sourcecompany->idprof2) $stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->transnoentities("ProfId2").": ".$outputlangs->convToOutputCharset($sourcecompany->idprof2);
			// Phone
