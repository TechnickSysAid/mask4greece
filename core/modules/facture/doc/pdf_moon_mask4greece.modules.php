...
/**
 *  \file       htdocs/core/modules/facture/doc/pdf_moon.modules.php
 *  \ingroup    facture
 *  \brief      File of class to generate customers invoices based on sponge model
 */
...
dol_include_once('/mask4greece/lib/mask4greece.lib.php');

/**
 *	Class to manage PDF invoice template moon
 */
class pdf_moon_mask4greece extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "moon";
		$this->description = $langs->trans('PDFMoonDescription');
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if ($object->type == 2 && !empty($object->fk_facture_source)) {
			...
			// Το παραπάνω βγάζει "Τιμολόγιο Διόρθωσης", άλλαξα την μετάφραση στο Transifex και θα το επαναφέρουμε στην 18.0.0
			$pdf->MultiCell($w +5, 3, "Σχετικό παραστατικό".' : '.$outputlangs->convToOutputCharset($objectreplaced->ref), '', 'L');
		...
		$pdf->SetFont('','', $default_font_size - 1); 
		$pdf->MultiCell($w, 3, $title." : ".dol_print_date($object->date, "day", false, $outputlangs, true), '', 'L');
			...
			if (empty($conf->global->MAIN_PDF_NO_SENDER_FRAME)) {
				...
				$pdf->SetFont('', '', $default_font_size - 2);
			        $pdf->SetXY($posx -5, $posy); 
                                $pdf->SetFillColor(255,255,255); 
				$pdf->MultiCell($widthrecbox +25, $hautcadre, "", 0, 'R', 1); 
                         }
		...
		if (empty($conf->global->MAIN_PDF_HIDE_CUSTOMER_CODE) && $object->thirdparty->code_client) {
			...
			// Show recipient profession
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->SetXY($posx, $posy + 11);
			//$pdf->SetXY($posx+2,$posy+4+(dol_nboflines_bis($carac_client_name,50)*4)); //
			///$pdf->MultiCell($widthrecbox - 2, 2, $carac_client, 0, $ltrdirection);
			$pdf->MultiCell($widthrecbox, 0, $outputlangs->transnoentities("Profession").': '.$outputlangs->transnoentities($object->thirdparty->idprof1, 0, 28), 0, 'L'); //	
			...
			if (getDolGlobalInt('INVOICE_SHOW_SHIPPING_ADDRESS')) {
				...
				if (isset($carac_client_shipping)) {
					$posy += $hautcadre; 
					...
					$pdf->Rect($posx, $posy -3, $widthrecbox, $hautcadre -19);
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
        ...
        // Οι στήλες είναι στο σωστό μέγεθος χωρίς να σπάνε οι ετικέτες
