...
/**
 *	\file       htdocs/core/modules/facture/mod_facture_moon.php
 *	\ingroup    facture
 *	\brief      File containing class for numbering module Moon
 */
require_once DOL_DOCUMENT_ROOT.'/core/modules/facture/modules_facture.php';


/**
 *	Class of numbering module Moon for invoices
 */
class mod_facture_moon extends ModeleNumRefFactures
{
	/**
	...
	public function info()
	{
		...
		$texte .= '<input type="hidden" name="maskconstinvoice" value="FACTURE_MOON_MASK_INVOICE">';
		$texte .= '<input type="hidden" name="maskconstcredit" value="FACTURE_MOON_MASK_CREDIT">';
		...
		$texte .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="maskinvoice" value="'.getDolGlobalString("FACTURE_MOON_MASK_INVOICE").'">', $tooltip, 1, 1).'</td>';
		...
		$texte .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="maskcredit" value="'.getDolGlobalString("FACTURE_MOON_MASK_CREDIT").'">', $tooltip, 1, 1).'</td>';
		$texte .= '</tr>';
	...
	public function getNextValue($objsoc, $invoice, $mode = 'next')
	{
        ...
        if (is_object($invoice) && $invoice->type == 2) {
			$mask = getDolGlobalString('FACTURE_MOON_MASK_CREDIT');
		} else {
			$mask = getDolGlobalString('FACTURE_MOON_MASK_INVOICE');
		}
		if (!$mask) {
