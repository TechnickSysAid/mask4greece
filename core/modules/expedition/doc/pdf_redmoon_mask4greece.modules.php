...
/**
 *	\file       htdocs/core/modules/expedition/doc/pdf_redmoon_mask4greece.modules.php
 *	\ingroup    expedition
 ...
 */
class pdf_redmoon_mask4greece extends ModelePdfExpedition
{
    /**
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// Το παραπάνω βγάζει "Φύλλο αποστολής" αντί για "Δελτίο αποστολής" 
		} else {
		...
		$pdf->MultiCell($w, 4, $outputlangs->transnoentities("Shipmentpurpose")." : ".$shippurpose, '', 'L');  // Σκοπός Διακίνησης
                ...
                $pdf->SetFont('','', $default_font_size - 1); 				
		$pdf->SetXY($posx +13, $posy + 32);
		$pdf->MultiCell($w, 4, $outputlangs->transnoentities("Shipmenttime")." : ".$stime, '', 'L');   // Ημ. & Ώρα αποστολής
                $posy += 4;
                $pdf->SetFont('','', $default_font_size - 1); 
		$pdf->SetXY($posx +13, $posy + 32);
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox, 2, $carac_client_name."\n", 0, 'L');
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'SHIPPING_FREE_TEXT', $this->emetteur=0, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
