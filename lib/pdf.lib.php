 ...
 */
function pdf_getInstancegr($format='',$metric='mm',$pagetype='P')
{
	global $conf;
 ...
 */
function pdf_getPDFFontgr($outputlangs)
{
	global $conf;
 ...
 */
function pdf_getPDFFontSizegr($outputlangs)
{
 ...
 * @return	number
 */
function pdf_getHeightForLogogr($logo, $url = false)
{
global $conf;
 ...
 */
function pdf_build_addressgr($outputlangs,$sourcecompany,$targetcompany='',$targetcontact='',$usecontact=0,$mode='source')
{
	global $conf;
	...
	if ($mode == 'source')
	{
		...
		if (empty($conf->global->MAIN_PDF_DISABLESOURCEDETAILS))
		{
            ...
            // Prof Id 1
			if ($sourcecompany->idprof1) $stringaddress .= ($stringaddress ? "" : '' ).$outputlangs->transnoentities("ProfId1").": ".$outputlangs->convToOutputCharset($sourcecompany->idprof1);
			// IntraCommunautary VAT
			if ($sourcecompany->tva_intra) $stringaddress .= ($stringaddress ? " - " : '' ).$outputlangs->transnoentities("VATIntraShort").": ".$outputlangs->convToOutputCharset($sourcecompany->tva_intra);
            // Prof Id 2
			if ($sourcecompany->idprof2) $stringaddress .= ($stringaddress ? " - " : '' ).$outputlangs->transnoentities("ProfId2").": ".$outputlangs->convToOutputCharset($sourcecompany->idprof2);
			// Phone
            ...
            // Fax
			if ($sourcecompany->fax) $stringaddress .= ($stringaddress ? " - " : '' ).$outputlangs->transnoentities("Fax").": ".$outputlangs->convToOutputCharset($sourcecompany->fax);
			// EMail
			if ($sourcecompany->email) $stringaddress .= ($stringaddress ? " - " : '' ).$outputlangs->transnoentities("Email").": ".$outputlangs->convToOutputCharset($sourcecompany->email);
			// Web
			if ($sourcecompany->url) $stringaddress .= ($stringaddress ? " - " : '' ).$outputlangs->transnoentities("Web").": ".$outputlangs->convToOutputCharset($sourcecompany->url);
 ...
 *      @return	void
 */
function pdf_pageheadgr(&$pdf,$outputlangs,$page_height)
{
	global $conf;
 ...
 *      @return	void
 */
function pdf_watermarkgr(&$pdf, $outputlangs, $h, $w, $unit, $text)
{
 ...
 */
function pdf_bankgr(&$pdf,$outputlangs,$curx,$cury,$account,$onlynumber=0,$default_font_size=10)
{
	global $mysoc, $conf;
	...
	if ($usedetailedbban)
	{
		...
		foreach ($fieldstoshow as $val)
		{
			if ($val == 'bank')
			{
				...
				$pdf->SetFont('','',$default_font_size - 4);$pdf->MultiCell($tmplength, 3, $outputlangs->convToOutputCharset($account->code_banque), 0, 'C', 0);
				$pdf->SetXY($curx, $cury+1);
				$curx+=$tmplength;
				$pdf->SetFont('','B',$default_font_size - 5);$pdf->MultiCell($tmplength, 3, $outputlangs->transnoentities("BankCode"), 0, 'C', 0);
			...
			if ($val == 'number')
			{
				...
				$tmplength=24;
				$pdf->SetXY($curx, $cury+5);
				$pdf->SetFont('','',$default_font_size - 4);$pdf->MultiCell($tmplength, 3, $outputlangs->convToOutputCharset($account->number), 0, 'C', 0);
				$pdf->SetXY($curx, $cury+1);
				$curx+=$tmplength;
				$pdf->SetFont('','B',$default_font_size - 5);$pdf->MultiCell($tmplength, 3, $outputlangs->transnoentities("BankAccountNumber"), 0, 'C', 0);
				if (empty($onlynumber)) $pdf->line($curx, $cury+1, $curx, $cury+8);
			}
			if ($val == 'key')
			{
 ...
 */
function pdf_pagefootgr(&$pdf,$outputlangs,$paramfreetext,$fromcompany,$marge_basse,$marge_gauche,$page_hauteur,$object,$showdetails=0,$hidefreetext=0)
{
	global $conf,$user;
	...
	if (strtolower(pdf_getPDFFontgr($outputlangs)) == 'helvetica')
	{
		$pdf->SetXY(-20,-$posy);
 ...
 *	@return	float                           The Y PDF position
 */
function pdf_writeLinkedObjectsgr(&$pdf,$object,$outputlangs,$posx,$posy,$w,$h,$align,$default_font_size)
{
	$linkedobjects = pdf_getLinkedObjectsgr($object,$outputlangs);
 ...
 * 	@return	void
 */
function pdf_writelinedescgr(&$pdf,$object,$i,$outputlangs,$w,$h,$posx,$posy,$hideref=0,$hidedesc=0,$issupplierline=0)
{
	...
	if (is_object($hookmanager) && ( (isset($object->lines[$i]->product_type) && $object->lines[$i]->product_type == 9 && ! empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line) ) )
	{
		...
		$action='';
		$reshook=$hookmanager->executeHooks('pdf_writelinedescgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
	}
	if (empty($reshook))
	{
		$labelproductservice=pdf_getlinedescgr($object,$i,$outputlangs,$hideref,$hidedesc,$issupplierline);
		// Description
 ...
 */
function pdf_getlinedescgr($object,$i,$outputlangs,$hideref=0,$hidedesc=0,$issupplierline=0)
{
 ...
 * 	@return	void
 */
function pdf_getlinenumgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
 ...
 * 	@return	void
 */
function pdf_getlinerefgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
 ...
 * 	@return	void
 */
function pdf_getlineref_suppliergr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
 ...
 * 	@return	string
 */
function pdf_getlinevatrategr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if (is_object($hookmanager) && (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line)))
	{
		...
		$action='';
		return $hookmanager->executeHooks('pdf_getlinevatrategr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
	}
	else
	{
 ...
 * 	@return	string
 */
function pdf_getlineupexcltaxgr($object,$i,$outputlangs,$hidedetails=0)
{
	...
	if (is_object($hookmanager) && (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line)))
	{
		...
		$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
		$action='';
		return $hookmanager->executeHooks('pdf_getlineupexcltaxgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
	}
	else
	{
 ...
 *  @return	void
 */
function pdf_getlineupwithtaxgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if (is_object($hookmanager) && (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line)))
	{
		...
		{
			if (method_exists($modules[$special_code],'pdf_getlineupwithtaxgr')) return $modules[$special_code]->pdf_getlineupwithtaxgr($object,$i,$outputlangs,$hidedetails);
		}
	}
	else
	{
 ...
 *  @return	string
 */
function pdf_getlineqtygr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if ($object->lines[$i]->special_code != 3)
	{
		if (is_object($hookmanager) && (( $object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code) ) || ! empty($object->lines[$i]->fk_parent_line) ) )
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlineqtygr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 * 	@return	string
 */
function pdf_getlineqty_askedgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if ($object->lines[$i]->special_code != 3)
	{
		if (is_object($hookmanager) && (( $object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code) ) || ! empty($object->lines[$i]->fk_parent_line) ) )
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlineqty_askedgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 * 	@return	string
 */
function pdf_getlineqty_shippedgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if ($object->lines[$i]->special_code != 3)
	{
		if (is_object($hookmanager) && (( $object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code) ) || ! empty($object->lines[$i]->fk_parent_line) ) )
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlineqty_shippedgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 * 	@return	void
 */
function pdf_getlineqty_keeptoshipgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if ($object->lines[$i]->special_code != 3)
	{
		if (is_object($hookmanager) && (( $object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code) ) || ! empty($object->lines[$i]->fk_parent_line) ) )
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlineqty_keeptoshipgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 * 	@return	string
 */
function pdf_getlineremisepercentgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	if ($object->lines[$i]->special_code != 3)
	{
		if (is_object($hookmanager) && ( ($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code) ) || ! empty($object->lines[$i]->fk_parent_line) ) )
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlineremisepercentgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 */
function pdf_getlinetotalexcltaxgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $conf, $hookmanager;
	...
	else
	{
		if (is_object($hookmanager) && (($object->lines[$i]->product_type == 9 && ! empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line)))
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlinetotalexcltaxgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 */
function pdf_getlinetotalwithtaxgr($object,$i,$outputlangs,$hidedetails=0)
{
	global $hookmanager;
	...
	else
	{
		if (is_object($hookmanager) && (($object->lines[$i]->product_type == 9 && ! empty($object->lines[$i]->special_code)) || ! empty($object->lines[$i]->fk_parent_line)))
		{
			...
			$parameters = array('i'=>$i,'outputlangs'=>$outputlangs,'hidedetails'=>$hidedetails,'special_code'=>$special_code);
			$action='';
			return $hookmanager->executeHooks('pdf_getlinetotalwithtaxgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
		}
		else
		{
 ...
 * 	@return	void
 */
function pdf_getTotalQtygr($object,$type,$outputlangs)
{
	global $hookmanager;
 ...
 */
function pdf_getLinkedObjectsgr($object,$outputlangs)
{
	global $hookmanager;
	...
	if (is_object($hookmanager))
	{
		...
		$action='';
		$hookmanager->executeHooks('pdf_getLinkedObjectsgr',$parameters,$object,$action);    // Note that $action and $object may have been modified by some hooks
 ...
 */
function pdf_getSizeForImagegr($realpath)
{
	global $conf;
